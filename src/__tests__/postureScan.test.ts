...
describe("runPostureScan — filesystem-based signals", () => {
  ...
  it("reports absent (never unknown) when files genuinely don't exist — always observable", async () => {
...
describe("runPostureScan — branch protection", () => {
  ...
  it("reports unknown/insufficient_permission — NOT absent — on the real 403 'Resource not accessible by integration' shape", async () => {
...
describe("runPostureScan — dependabot alerts enabled/disabled", () => {
  ...
  it("404 -> absent", async () => {
    ...
    const result = await runPostureScan({ workspace: "/ws", fileSystem: NO_FILES, repository: "acme/widgets", token: "tok", fetchImpl });
    expect(find(result.findings, "dependabot_alerts_enabled").status).toBe("absent");
  });
...
// ─── org-level 2FA/SSO enforcement ──────────────────────────────────────────

describe("runPostureScan — org 2FA/SSO enforcement", () => {
  it("reports a real result when the org endpoint genuinely returns the field", async () => {
    ...
    const result = await runPostureScan({ workspace: "/ws", fileSystem: NO_FILES, repository: "acme/widgets", token: "tok", fetchImpl });
    const f = find(result.findings, "org_2fa_sso_enforcement");
    expect(f.observable).toBe(true);
    expect(f.status).toBe("absent");
    expect(f.reason).toBe("observed");
  });

  it("reports not_observable_by_repo_token (never guesses) on the structural 403 a repo token gets", async () => {
    const fetchImpl = fetchRouting([
      { match: REPO_URL, status: 200, body: { default_branch: "main", owner: { type: "Organization" } } },
      { match: PROTECTION_URL, status: 403 },
      { match: ALERTS_URL, status: 403 },
      { match: ORG_URL, status: 403, text: JSON.stringify({ message: "Must have admin rights to Repository." }) },
    ]);
    const result = await runPostureScan({ workspace: "/ws", fileSystem: NO_FILES, repository: "acme/widgets", token: "tok", fetchImpl });
    const f = find(result.findings, "org_2fa_sso_enforcement");
    expect(f.observable).toBe(false);
  ...
  it("reports not_observable_by_repo_token even on a 200 that omits the field", async () => {
    const fetchImpl = fetchRouting([
      { match: REPO_URL, status: 200, body: { default_branch: "main", owner: { type: "Organization" } } },
      { match: PROTECTION_URL, status: 403 },
      { match: ALERTS_URL, status: 403 },
      { match: ORG_URL, status: 200, body: { login: "acme" } },
    ]);
    const result = await runPostureScan({ workspace: "/ws", fileSystem: NO_FILES, repository: "acme/widgets", token: "tok", fetchImpl });
    const f = find(result.findings, "org_2fa_sso_enforcement");
    expect(f.status).toBe("unknown");
...
describe("runPostureScan — summary counts", () => {
  it("tallies observed / not-observable / present / absent correctly", async () => {
    ...
    expect(result.observed_count + result.not_observable_count).toBe(result.findings.length);
...
describe("renderPostureStepSummary", () => {
  it("renders a markdown table naming every finding, its status, and its reason", () => {
    const findings: PostureFinding[] = [
      ...
      { id: "branch_protection", label: "Branch protection (main)", observable: false, status: "unknown", reason: "insufficient_permission", detail: "Needs administration:read." },
    ];
